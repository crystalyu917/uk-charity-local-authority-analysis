...
Code lives in this folder; default input and output files live in
[`data/core/`](../../../data/core/). Configure core inputs only in this folder's
[`config.py`](config.py).

## Configure inputs

Core data paths and download settings live in
[`config.py`](config.py).
...
| Location | Purpose |
| --- | --- |
| `config.py` | Core input paths, ONS endpoint, and output settings |
| `charity.py` | Register preparation, saved runs, and removal aggregation |
| `datasets/` | Shared download and archive extraction helpers |
| `datasets/ons_postcode_lookup/` | ONS download, extraction, and lookup preparation |
| `tests/` and nested `tests/` folders | Tests beside the modules they cover |
...
```powershell
uv run python -m unittest discover -s src/uk_charity_local_authority_analysis/core -t src -v
```
