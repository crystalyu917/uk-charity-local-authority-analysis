...
Archived for reference; current scripts do not import these helpers.
The supported file-based pipeline is projects.council_asset_sales.pipeline.panel.
"""
...
LA_CAPITAL_RECEIPTS_COL = "EandR1_alltot_rectot"
...
def clean_la_capital_receipts(la_capital_receipts: pl.DataFrame) -> pl.DataFrame:
    if LA_CAPITAL_RECEIPTS_COL not in la_capital_receipts.columns:
        raise ValueError(
            f"Missing expected capital receipts column: {LA_CAPITAL_RECEIPTS_COL}"
        )

    return (
        la_capital_receipts.select(
            ...
            pl.col(LA_CAPITAL_RECEIPTS_COL)
            .cast(pl.Utf8)
            .str.strip_chars()
            .str.replace_all(",", "")
            ...
            .alias("la_capital_receipts_thousand"),
        )
        ...
        .with_columns(
            ...
            (pl.col("la_capital_receipts_thousand") / 1000).alias("value"),
        )
        .select(
            "financial_year",
...
def create_complete_panel(
    la_capital_receipts: pl.DataFrame,
    charity_df: pl.DataFrame,
) -> pl.DataFrame:
    receipts = clean_la_capital_receipts(la_capital_receipts)
    ...
    sizes = pl.DataFrame({"size_category": config.CHARITY_SIZE_CATEGORIES})
    ...
    panel = (
        ...
        .filter(pl.col("financial_year").is_between(
            config.PANEL_START_YEAR, config.PANEL_END_YEAR
        ))
        .sort(
            [
                "local_authority",
