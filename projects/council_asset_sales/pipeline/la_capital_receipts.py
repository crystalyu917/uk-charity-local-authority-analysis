...
from projects.council_asset_sales.pipeline.la_receipts import (
...
def prepare_la_capital_receipts(
    la_capital_receipts: pl.LazyFrame,
) -> pl.LazyFrame:
    ...
    derivative.
    """
    _require_columns(la_capital_receipts, _REQUIRED_COLUMNS)

    submitted_lads = (
        la_capital_receipts.filter(
        ...
        .select(
            ...
            .cast(pl.Int64)
            .alias("la_capital_receipts_gbp_thousands"),
        )
        .with_columns(
    ...
    source_record = (
        ...
        .alias("la_capital_receipts_sources")
    )
    return (
        submitted_lads.group_by("financial_year", "local_authority_code")
        .agg(
            pl.col("la_capital_receipts_gbp_thousands").sum(),
            source_record,
        )
        .with_columns(
            (pl.col("la_capital_receipts_gbp_thousands").cast(pl.Float64) / 1_000).alias(
...
def scan_la_capital_receipts(
    dest_dir: Path | None = None, *, input_filepath: Path | None = None,
) -> pl.LazyFrame:
    """Build the prepared capital-receipts query without materialising it."""
    return prepare_la_capital_receipts(scan_la_receipts(dest_dir, input_filepath=input_filepath))


def load_la_capital_receipts(
    dest_dir: Path | None = None, *, input_filepath: Path | None = None,
) -> pl.DataFrame:
    """Materialise the prepared capital-receipts dataset."""
    return scan_la_capital_receipts(dest_dir, input_filepath=input_filepath).collect(engine="streaming")
